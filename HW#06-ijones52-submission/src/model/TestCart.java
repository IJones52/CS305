...
class TestCart {
    ...
    /**
     * A method that tests add for more coverage
     * */
     @Test
    void testAdd() {
        cart1.add(order1);
    ...
    /**
     * A method that tests the membership feature for more coverage
     * */
    @Test
    void testMembership() {
        //Should be false to begin with
        assertEquals(false, cart1.getMembershipStatus());
        //Change it to true
        cart1.setMembership(true);
        assertEquals(true, cart1.getMembershipStatus());
        //Change it back to false
        cart1.setMembership(false);
        assertEquals(false,cart1.getMembershipStatus());
    }
    
    /**
     * A method that tests total calculation for more coverage
     * */
    @Test
    void testCalculateTotal() {
        ...
        //Should be 6
        cart1.setMembership(true);
        item1 = new Item("Thing", new BigDecimal(2), 4, new BigDecimal(3));
        order1 = new ItemOrder(item1, 6);
        cart1.add(order1);
        assertEquals(new BigDecimal(7),cart1.calculateTotal());
        //Should be 4
        cart1.clear();
        order1 = new ItemOrder(item1, 2);
        cart1.add(order1);
        assertEquals(new BigDecimal(4),cart1.calculateTotal());
